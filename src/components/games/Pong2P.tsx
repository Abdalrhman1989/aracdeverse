...
const W=400,H=300,PADDLE_H=60,PADDLE_W=10,BALL_R=7,WIN_SCORE=7

export default function Pong2P({onScoreUpdate,onGameOver,onGameStart,isPaused}:GameComponentProps){
  ...
  const loop=useCallback(()=>{
    ...
    // Paddle collisions
    if(b.x-BALL_R<=20+PADDLE_W && b.y>s.p1.y && b.y<s.p1.y+PADDLE_H && b.vx<0){
      b.vx=Math.abs(b.vx)*1.05;b.vy+=((b.y-(s.p1.y+PADDLE_H/2))/PADDLE_H)*3
    }
    if(b.x+BALL_R>=W-20 && b.y>s.p2.y && b.y<s.p2.y+PADDLE_H && b.vx>0){
      b.vx=-Math.abs(b.vx)*1.05;b.vy+=((b.y-(s.p2.y+PADDLE_H/2))/PADDLE_H)*3
    }
    // Score
    if(b.x<0){s.p2.score++;setScores([s.p1.score,s.p2.score]);b.x=W/2;b.y=H/2;b.vx=4;b.vy=3*(Math.random()<0.5?1:-1)}
    if(b.x>W){s.p1.score++;setScores([s.p1.score,s.p2.score]);b.x=W/2;b.y=H/2;b.vx=-4;b.vy=3*(Math.random()<0.5?1:-1)}
    // Cap speed
    const spd=Math.sqrt(b.vx*b.vx+b.vy*b.vy);if(spd>12){b.vx=b.vx/spd*12;b.vy=b.vy/spd*12}
    if(s.p1.score>=WIN_SCORE||s.p2.score>=WIN_SCORE){
      phaseRef.current="gameover";setPhase("gameover");onGameOver(Math.max(s.p1.score,s.p2.score))
    }
    draw()
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="relative">
        ...
        {phase==="start"&&<div className="absolute inset-0 flex flex-col items-center justify-center bg-space-bg/90 rounded-lg">
          ...
          <h2 className="text-2xl font-orbitron text-neon-blue mb-2">PONG 2-PLAYER</h2>
          <p className="text-space-muted mb-2 text-sm">P1: W/S keys | P2: ↑/↓ keys</p>
          <p className="text-space-muted mb-6 text-sm">First to {WIN_SCORE} wins!</p>
        ...
        {phase==="gameover"&&<div className="absolute inset-0 flex flex-col items-center justify-center bg-space-bg/90 rounded-lg">
          ...
          <h2 className="text-xl font-orbitron text-neon-yellow mb-1">{scores[0]>=WIN_SCORE?"PLAYER 1 WINS!":"PLAYER 2 WINS!"}</h2>
